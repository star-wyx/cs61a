...
def pingpong(n):
    ...
    def recursion(pre_pre, pre, i, n):
        ...
        if num_sevens(i - 1) != 0 or (i - 1) % 7 == 0:
            # res = pre_pre
            # pre_pre, pre = pre, res
            return recursion(pre, pre_pre, i + 1, n)
        elif pre > pre_pre:
            # res = pre + 1
            # pre_pre, pre = pre, res
            return recursion(pre, pre + 1, i + 1, n)
        else:
            # res = pre - 1
            # pre_pre, pre = pre, res
            return recursion(pre, pre - 1, i + 1, n)

    return recursion(1, 2, 3, n)
...
def count_change(amount):
    ...
    def partition(n, m):
        if n == 1 or n == 0:
            ...
            return partition(n - m, m) + partition(n, next_lager(m))

    def next_lager(x, i=1):
        if x == 1:
            return 1
        if 2 * i >= x:
            return i
        else:
            return next_lager(x, 2 * i)

    return partition(amount, next_lager(amount))
