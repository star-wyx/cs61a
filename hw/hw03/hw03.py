...
def replace_leaf(t, old, new):
    ...
    newt = []
    for m in branches(t):
        newt.append(replace_leaf(m, old, new))
    return tree(label(t), newt)
...
def balanced(m):
    ...
    if is_mobile(end(left(m))):
        end_left = balanced(end(left(m)))
    if is_mobile(end(right(m))):
        end_right = balanced(end(right(m)))

    return length(left(m)) * total_weight(end(left(m))) == length(right(m)) * total_weight(
        end(right(m))) and end_left and end_right
...
def totals_tree(m):
    ...
    if is_weight(m):
        return tree(size(m))
    lst.append(totals_tree(end(left(m))))
    lst.append(totals_tree(end(right(m))))
    res = tree(total_weight(m), lst)
    return res
...
def mul_church(m, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    # return lambda f: lambda x:m(n(f))(n(f)(x)) // incorrect
