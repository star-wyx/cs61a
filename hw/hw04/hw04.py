...
def make_withdraw(balance, password):
    ...
    def run(cost, pwd):
        ...
        if len(record) >= 3:
            return "Your account is locked. Attempts: ['" + record[0] + "', '" + record[1] + "', '" + record[2] + "']"
        if pwd != password:
            record.append(pwd)
            if len(record) <= 3:
                return 'Incorrect password'
            else:
                return "Your account is locked. Attempts: ['" + record[0] + "', '" + record[1] + "', '" + record[
                    2] + "']"
        else:
            if cost > balance:
...
class VendingMachine:
    ...
    def vend(self):
        if self.number == 0:
            ...
            if self.current < self.value:
                return 'You must deposit ${0} more.'.format(self.value - self.current)
                ...
                self.number -= 1
                string = 'Here is your {0} and ${1} change.'.format(self.name, self.current - self.value)
                self.current = 0
                return string
    ...
    def deposit(self, money):
        if self.number == 0:
            ...
            self.current += money
            return 'Current balance: ${0}'.format(self.current)
