...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and insect.already:
                return
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def within(distance):
            if self.max_range is None and self.min_range is None:
                return True
            if self.max_range >= distance >= self.min_range:
                return True
            return False
        ...
        distance = 0
        while p is not hive:
            if within(distance) and len(p.bees) > 0:
                break
            p = p.entrance
            distance += 1
        if p is hive:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        self.counter = self.time_to_digest
        # END Problem 6
        ...
        if not self.already:
            ...
            while p is not None:
                ...
                if p.ant is not None:
                    if isinstance(p.ant, TankAnt):
                        if not p.ant.enforce:
                            p.ant.enforce = True
                            p.ant.damage *= 2
                        if p.ant.contained_ant is not None:
                            if not p.ant.contained_ant.enforce:
                                p.ant.contained_ant.enforce = True
                                p.ant.contained_ant.damage *= 2
                    elif isinstance(p.ant, BodyguardAnt):
                        if p.ant.contained_ant is not None:
                            if not p.ant.contained_ant.enforce:
                                p.ant.contained_ant.enforce = True
                                p.ant.contained_ant.damage *= 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        ...
        while p != hive:
            for bee in p.bees:
                res[bee] = distance
            if p.ant is not None and p.ant != self:
                res[p.ant] = distance
            p = p.entrance
            distance += 1
        return res
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        return self.damage - distance * 0.2 - 0.05 * self.insects_shot
        # END Problem OPTIONAL
